...
public class PanelSalesList extends JPanel implements DocumentListener, ActionListener, ItemListener {
   ...
   private void initComponents() {
      ...
      btnRank = new JButton("TOP5");
      btnRank.addActionListener(this);
      
      btnToday = new JButton("오늘날짜보기");
   ...
   public void setListToday() {
      ...
      itemList = service.selectSalesStatusByDate(format.format(now));
      setListRank();
      panel_2.setItemList(itemList);
      panel_2.reloadData();
   ...
   public void setListTop() {
      //List<SalesStatus> listRank = service.selectSalesStatusByAll();
      try {
         List<SalesStatus> listRank = itemList;
         itemList = new ArrayList<SalesStatus>();
         for(int i =0; i<5; i++) {
            itemList.add(listRank.get(i));
         }
   ...
   protected void actionPerformedBtnRank(ActionEvent e) { //상위5개보기
      //setDatePickerText();
      setListTop();
      //setCmbClear();
