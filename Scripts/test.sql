...
from sale s left join food f on s.fd_no = f.fd_no where s.sale_time like "2019-07-25%"
group by s.fd_no
...
select sale_time from sale where sale_time like "2019-07-24%";

select no,sale_no,sale_time,order_cnt,order_kind,sale_type,f.name, m.name
		from sale s left join food f on s.fd_no = f.fd_no join member as m on s.mb_no = m.mb_no where sale_time like "2019-07-24%";
